...
public class Display extends JFrame {
    ...
    public class GraphPanel extends JPanel {
        ...
        public void paintGraph(Graphics g) {
            // for each edge in the graph, use paintEdge(g, edge.source,
            // edge.target, edge.distance, Color.LIGHT_GRAY, DEFAULT_THICKNESS, 255);
            // to print the edge.
            //todo:magic number
            List<Edge> edges = graph.getAllEdges();
            for(Edge e: edges){
                paintEdge(g,e.getSource(),e.getTarget(),e.getDistance(),
